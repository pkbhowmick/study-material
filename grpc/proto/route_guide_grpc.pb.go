...
func (c *routeGuideClient) ListFeature(ctx context.Context, in *Rectangle, opts ...grpc.CallOption) (RouteGuide_ListFeatureClient, error) {
	stream, err := c.cc.NewStream(ctx, &RouteGuide_ServiceDesc.Streams[0], "/proto.RouteGuide/ListFeature", opts...)
	if err != nil {
		return nil, err
	}
...
func (x *routeGuideRouteChatClient) CloseAndRecv() (*RouteNote, error) {
	if err := x.ClientStream.CloseSend(); err != nil {
		return nil, err
	}
	m := new(RouteNote)
...
func (x *routeGuideRouteChatServer) SendAndClose(m *RouteNote) error {
	return x.ServerStream.SendMsg(m)
}
